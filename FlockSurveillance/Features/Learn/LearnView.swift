...
private struct LearnArticle: Identifiable {
    ...
    static let all: [LearnArticle] = [
        ...
        LearnArticle(
            ...
            body: "Mapped Camera Pins plots ALPR nodes that volunteers have tagged in OpenStreetMap. Coverage is uneven by design: it reflects what people have documented, not a vendor’s private inventory. The map HUD shows fetch confidence (share of pins with a tagged OSM direction, and freshness), and soft-clears pins after a successful refresh no longer returns them. AR Camera Sight shows those same mapped locations in the street — not a live feed from any camera network."
        ),
        LearnArticle(
            id: "sensor-atlas",
